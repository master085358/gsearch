...
uv run github-fetch fetch-file-content --graphql

# REST mode (slow -- one API call per file)
uv run github-fetch fetch-file-content

# Custom batch size and paths
uv run github-fetch fetch-file-content --graphql --batch-size 50 --db /path/to/db.db --content-dir /path/to/content
```
...
# REST mode (slow -- one API call per repo, ~0.6 repos/sec)
uv run github-fetch fetch-repo-metadata

# Custom batch size and output
uv run github-fetch fetch-repo-metadata --graphql --batch-size 50 --db /path/to/db.db -o metadata.json
```
